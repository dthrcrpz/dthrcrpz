<template>
  <section class="technicalities relative rounded-b-3xl overflow-hidden">
    ...
    <div class="container py-10 lg:py-20 relative">
      ...
      <div class="mt-10 md:mt-20 flex flex-row flex-wrap justify-between" v-for="(techStack, key) in techStacks" :key="key">
        ...
        <div class="flex flex-row flex-wrap w-full md:w-3/5">
          <div class="stack" v-for="(stack, key) in techStack.stacks" :key="key">
            <div class="icon w-[40px] h-[40px] flex justify-center items-end mb-5 transition-transform duration-300 {(stack.bg) ? 'bg-white px-1 py-1 rounded-md' : null } relative">
              <small class="absolute px-2 py-1 bg-secondary text-[10px] rounded-xl bg-opacity-50 top-0 -rotate-6 text-white" v-if="stack.learning">LEARNING</small>
              <small class="absolute px-2 py-1 bg-tertiary text-[10px] rounded-xl bg-opacity-50 top-0 -rotate-6 text-white" v-if="stack.onQueue">QUEUED</small>
